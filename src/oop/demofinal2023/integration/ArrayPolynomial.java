...
public class ArrayPolynomial extends AbstractPolynomial {
    ...
    public ArrayPolynomial plus(ArrayPolynomial another) {
        ...
        for (int i = 0; i <= another.degree(); i++) {
            resultCoefficients[i] = resultCoefficients[i] + another.coefficient(i);
        }
    ...
    public ArrayPolynomial minus(ArrayPolynomial another) {
        /* TODO */
        double[] resultCoefficients = Arrays.copyOf(coefficients(), Math.max(another.degree() + 1, degree()));
        for (int i = 0; i <= another.degree(); i++) {
            resultCoefficients[i] = resultCoefficients[i] - another.coefficient(i);
        }
        coefficents = resultCoefficients;
    ...
    public ArrayPolynomial multiply(ArrayPolynomial another) {
        /* TODO */
        double[] resultCoefficients = new double[another.degree() + degree() + 1];
        for (int i = 0; i <= degree(); i++) {
            for (int j = 0; j <= another.degree(); j++) {
                resultCoefficients[i + j] = resultCoefficients[i + j] + coefficient(i) * another.coefficient(j);
            }
        }
        coefficents = resultCoefficients;
        size = coefficents.length;
